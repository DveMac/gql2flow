...
program
    ...
    .option('--null-keys-from-values [nullKeysFromValues]', 'makes all keys nullable when value is', false)
    .option(
    ...
    .option('-w --whitelist <whitelist>', 'names of types to whitelist (comma delimited)', v => v.split(','), [])
    .action((path, options) => {
            ...
            if (isUrl(path)) {
                fetchUtils.fetchWithIntrospection(path, (err, schema) => {
                    if (err) {
                        ...
                        reject(new Error('Server replied with an invalid introspection schema'))
                    } else {
                        resolve(schema)
                    }
                })
